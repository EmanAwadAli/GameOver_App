...
export default function GamesBySort() {
  let { sort } = useParams();
  const [games, setGames] = useState([]);
  const [count, setCount] = useState(20);

  async function getGamesBysort(s) {
    let { data } = await axios
      .get(
        `https://free-to-play-games-database.p.rapidapi.com/api/games?sort-by=${s}`,
        {
          headers: {
            "X-RapidAPI-Key":
  ...
  return (
    <section className="sorted_games py-5">
